...
$(document).ready(function () {
    ...
    $('#saveLearningGroup').on('click', function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                var output = LoadPageError({HasError:false,Message:data.Message + "; You can proceed to assign courses to your group below"});
                $('.saveBox').append(output);
    ...
    $('#AssignCourse').on('click', function () {
        ...
        if (!parseInt($('#LearningGroupId').val()) || parseInt($('#LearningGroupId').val() == 0)) {
            ShowNotie({ HasError: true, Message: "No Learning Group has been created or selected" });
            $button.html(prev);
            $button.removeAttr('disabled');
            return;
        }
        ...
        $('#CourseList input:checkbox:checked').each(function (i, item) {
            var course = {
                CourseId: parseInt($(item).attr('data-id')),
                LearningGroupId: parseInt($('#LearningGroupId').val()),
                Required: $('#radioRecommend').is(':checked'),
                Assigned: $('#radioAssigned').is(':checked')
            }
            courses.push(course);
        });
        ...
        if (courses.length > 0) {
            $.ajax({
                ...
                async: true,
                type: "POST",
                url: "/api/LearningGroup/AssignCourseToLearnerGroup",
                data: { '': courses },
                success: function (data) {
                    ShowNotie(data);
                },
...
$(document).on("keypress", ".searchT", function (e) {
    if (e.which == 13) {
        var data = { KeyWords: $('.searchT').val(), Filter: null, FilterBy: null }
        var selectedId = $('#SelectLearningArea').val();

        if (selectedId != "0") {
            data.FilterBy = 1
            data.Filter = parseInt(selectedId)
        }


        $('#CourseList').html('<div class="leftOff loading"> <strong class="containers">Fetching Data...</strong></div>');
        $.get("/api/Course/SearchCourse", data, function (courses) {

            if (courses.Result.Courses.length == 0) {
                $('#CourseList').find('.loading strong').html('No results found');
                return;
            }
            courses.Result.Courses.forEach(function (course) {

                $('#CourseList').append('<div class="leftOff" data-area="' + course.LearningAreaId + '">' +
                    '<sup>' + course.LearningAreaId + '</sup>' +
