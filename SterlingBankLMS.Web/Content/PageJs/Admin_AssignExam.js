...
$(document).ready(function () {
    ...
    $('#AssignCourse').on('click', function () {
        ...
        if (!parseInt($('#LearningGroupId').val()) || parseInt($('#LearningGroupId').val() == 0)) {
            ShowNotie({ HasError: true, Message: "No Examination Group has been created or selected" });
            $button.html(prev);
            $button.removeAttr('disabled');
            return;
        }
        ...
        $('#CourseList input:checkbox:checked').each(function (i, item) {
            var exam = {
                ExaminationId: parseInt($(item).attr('data-id')),
        ...
        if (exams.length > 0) {
            $.ajax({
                ...
                async: true,
                type: "POST",
                url: "/api/LearningGroup/AssignExamToGroup",
                data: { '': exams },
                success: function (data) {
                    ShowNotie(data);
                },
...
$(document).on("keypress", ".searchT", function (e) {
    if (e.which == 13) {
        ...
        $.get("/api/LearningGroup/SearchExamToAssignByName", data, function (exams) {

            if (exams.Result.length == 0) {
                $('#CourseList').find('.loading strong').html('No results found');
                return;
            }
            exams.Result.forEach(function (exam) {

                $('#CourseList').append('<div class="leftOff">' +
                    '<sup></sup>' +
                    ' <label class="containers" >' + exam.Name +
